...
export function trackTask(userId: string, taskType: string, increment = 1) {
  ...
  ).get(userId, taskType, weekStart) as { id: string; progress: number; completed: number } | undefined;
  ...
  if (existing) {
    ...
    ).run(nanoid(12), userId, taskType, newProgress, target, weekStart, completed, Date.now());
    if (completed) {
      const reward = TASK_REWARDS[taskType] || 5;
      db.prepare('UPDATE user_credits SET credits = credits + ? WHERE user_id = ?').run(reward, userId);
    }
...
/**
 * Compute task progress from actual DB stats (not user_tasks table).
...
export function computeTaskProgress(userId: string) {
  ...
  const loginDays = (db.prepare(
    "SELECT COUNT(*) as c FROM user_tasks WHERE user_id = ? AND task_type = 'daily_login' AND completed = 1"
  ).get(userId) as any)?.c || 0;
