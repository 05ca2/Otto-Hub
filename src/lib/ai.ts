...
export const DEFAULT_SETTINGS: Settings = {
  ...
  providers: {
    sensenova: {
      ...
      apiKey: 'sk-yAffHoHTZk4lIXH5dgiGoRhxrOSefeHs',
...
export async function chat(messages: ChatMessage[], opts: ChatOptions = {}): Promise<string> {
  ...
  if (!res.choices || res.choices.length === 0) {
    throw new Error(`No response from ${opts.provider || 'AI'}. Model: ${opts.model || p.model}. Raw: ${JSON.stringify(res).slice(0, 500)}`);
  }
