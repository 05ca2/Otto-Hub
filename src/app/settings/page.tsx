  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div className="rounded-2xl border border-ink-100 dark:border-ink-700 bg-white dark:bg-ink-800 p-6">
        <h1 className="text-2xl font-semibold mb-1">{t(locale, 'settings.title')}</h1>
        <p className="text-ink-600 dark:text-ink-400 text-sm mb-5">
          {t(locale, 'settings.description')}
        </p>

        <div className="mb-5">
          <label className="block text-sm font-medium mb-1">{t(locale, 'settings.defaultProvider')}</label>
              ...
              <div key={k} className="border border-ink-100 dark:border-ink-700 rounded-lg p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className={`text-xs ${p?.hasApiKey ? 'text-green-600 dark:text-green-400' : 'text-ink-400 dark:text-ink-500'}`}>
                    {p?.hasApiKey ? t(locale, 'settings.configured') : t(locale, 'settings.noKey')}
                  </span>
                </div>
                <div className="space-y-3">
                  <div>
                    <label className="block text-xs font-medium mb-1 text-ink-600 dark:text-ink-400">{t(locale, 'settings.baseUrl')}</label>
                    <input
                      value={d.baseUrl}
                  ...
                  <div>
                    <label className="block text-xs font-medium mb-1 text-ink-600 dark:text-ink-400">{t(locale, 'settings.model')}</label>
                    <input
                      value={d.model}
                  ...
                  <div>
                    <label className="block text-xs font-medium mb-1 text-ink-600 dark:text-ink-400">{t(locale, 'settings.apiKey')}</label>
                    {k === 'custom' ? (
                      <div className="flex gap-2">
                        <input
                          ...
                          placeholder={p?.hasApiKey ? `••••••• (${t(locale, 'settings.alreadySet')})` : t(locale, 'settings.pasteKey')}
                      ...
                      <div className="flex items-center gap-2 text-sm text-ink-600 dark:text-ink-400">
                        ...
                        <span className="text-green-600 dark:text-green-400 text-xs">已内置，无需配置</span>
                      </div>
                    )}
                  ...
                  <details className="text-xs text-ink-600 dark:text-ink-400">
                    <summary className="cursor-pointer hover:underline">{t(locale, 'settings.quickRecipes')}</summary>
        ...
        {saved && <p className="text-sm text-green-600 mt-4 flex items-center gap-1.5"><CheckCircle2 className="w-4 h-4" /> {t(locale, 'settings.saved')}</p>}
        ...
        <div className="mt-6 flex items-center gap-3">
          ...
          >
            ...
            {saving ? 'Saving…' : t(locale, 'settings.saveAll')}
          </button>
        </div>
      </div>
    </div>
