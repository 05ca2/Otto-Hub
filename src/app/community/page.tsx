...
import { Plus, Users as UsersIcon, MessageSquare, Loader2, Search, Users } from 'lucide-react';
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold flex items-center gap-2">
            <UsersIcon className="w-5 h-5 text-accent-500" /> {t(locale, 'community.title')}
          </h1>
          <p className="text-ink-600 dark:text-ink-400 text-sm mt-1">{t(locale, 'community.description')}</p>
        </div>
        <div className="flex gap-2">
          <Link href="/community/groups" className="px-3 py-1.5 rounded border border-ink-200 dark:border-ink-600 hover:bg-ink-50 dark:hover:bg-ink-700 text-sm flex items-center gap-1.5">
            <Users className="w-4 h-4" /> {locale === 'zh' ? '群组' : 'Groups'}
          </Link>
          <Link href="/community/questions" className="px-3 py-1.5 rounded border border-ink-200 dark:border-ink-600 hover:bg-ink-50 dark:hover:bg-ink-700 text-sm flex items-center gap-1.5">
            <MessageSquare className="w-4 h-4" /> {t(locale, 'community.qaForum')}
          </Link>
          <button onClick={() => setShowCreate(true)} className="px-3 py-1.5 rounded bg-accent-500 text-white text-sm font-medium hover:bg-accent-600 flex items-center gap-1.5">
            <Plus className="w-4 h-4" /> {t(locale, 'community.createRoom')}
          </button>
        </div>
      </div>
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder={t(locale, 'community.searchRooms')}
      ...
      {loading ? (
        ...
        <>
          <section>
            <h2 className="text-lg font-semibold mb-3">{t(locale, 'community.yourRooms')}</h2>
            {filteredMine.length === 0 ? (
              <div className="rounded-xl border border-dashed border-ink-200 dark:border-ink-600 p-6 text-center text-sm text-ink-400 dark:text-ink-500">
                {search ? t(locale, 'community.noMatchingRooms') : t(locale, 'community.noRoomsJoined')}
              </div>
            ) : (
                ...
                {filteredMine.map((r) => (
                  <li key={r.id} className="rounded-xl border border-ink-100 dark:border-ink-700 bg-white dark:bg-ink-800 p-4">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs text-ink-400 dark:text-ink-500">{r.member_count} {t(locale, 'community.members')}</span>
                    </div>
                    ...
                    <div className="text-xs text-ink-400 dark:text-ink-500 mt-2 flex items-center gap-2">
                      <span>{t(locale, 'community.role')}: {r.role}</span>
                      {r.is_public ? <span className="px-1.5 py-0.5 rounded bg-green-100 text-green-700">{t(locale, 'community.public')}</span> : <span className="px-1.5 py-0.5 rounded bg-ink-100 dark:bg-ink-700 text-ink-600 dark:text-ink-300">{t(locale, 'community.private')}</span>}
          ...
          <section>
            <h2 className="text-lg font-semibold mb-3">{t(locale, 'community.publicRooms')}</h2>
            {filteredPublic.length === 0 ? (
              <p className="text-sm text-ink-400 dark:text-ink-500">{search ? t(locale, 'community.noMatchingPublic') : t(locale, 'community.noPublicRooms')}</p>
            ) : (
              <ul className="grid sm:grid-cols-2 gap-3">
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 bg-ink-900/40 flex items-center justify-center p-4" onClick={() => setShowCreate(false)}>
          <div className="bg-white dark:bg-ink-800 rounded-2xl shadow-2xl max-w-md w-full p-5" onClick={(e) => e.stopPropagation()}>
            <h3 className="font-semibold mb-3">{t(locale, 'community.createStudyRoom')}</h3>
            <div className="space-y-3">
              <div>
                <label className="block text-sm font-medium mb-1">{t(locale, 'community.roomName')}</label>
                <input value={name} onChange={(e) => setName(e.target.value)} className="w-full border border-ink-200 dark:border-ink-600 bg-white dark:bg-ink-900 dark:text-ink-100 rounded-md p-2 text-sm" />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">{t(locale, 'community.roomDesc')}</label>
                <textarea value={description} onChange={(e) => setDescription(e.target.value)} className="w-full border border-ink-200 dark:border-ink-600 bg-white dark:bg-ink-900 dark:text-ink-100 rounded-md p-2 text-sm min-h-[60px]" />
              </div>
              <label className="flex items-center gap-2 text-sm">
                <input type="checkbox" checked={isPublic} onChange={(e) => setIsPublic(e.target.checked)} />
                {t(locale, 'community.roomPublic')}
              </label>
            </div>
            <div className="flex justify-end gap-2 mt-4">
              ...
              <button onClick={create} disabled={!name.trim() || creating} className="px-3 py-1.5 rounded bg-accent-500 text-white text-sm font-medium hover:bg-accent-600 disabled:opacity-50 flex items-center gap-1.5">
                ...
                {creating ? t(locale, 'community.creating') : t(locale, 'community.createBtn')}
              </button>
            </div>
          </div>
        </div>
