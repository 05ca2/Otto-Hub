...
import { ArrowUp, ArrowDown, Loader2, Check, Send, ArrowLeft, Pencil, Trash2 } from 'lucide-react';
...
export default function PostDetailPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <section>
        <h2 className="text-lg font-semibold mb-3">{comments.length} {comments.length === 1 ? 'answer' : 'answers'}</h2>
        ...
        <div className="mt-4 rounded-xl border border-ink-100 dark:border-ink-700 bg-white dark:bg-ink-800 p-4">
          <textarea
            ...
            className="w-full border border-ink-200 dark:border-ink-600 dark:bg-ink-900 dark:text-ink-100 rounded-md p-2 text-sm min-h-[100px] font-mono"
          />
          ...
          <div className="bg-white dark:bg-ink-800 rounded-2xl shadow-2xl max-w-xl w-full p-5" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input value={editTitle} onChange={(e) => setEditTitle(e.target.value)} className="w-full border border-ink-200 dark:border-ink-600 dark:bg-ink-900 dark:text-ink-100 rounded-md p-2 text-sm" />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Body (Markdown OK)</label>
                <textarea value={editBody} onChange={(e) => setEditBody(e.target.value)} className="w-full border border-ink-200 dark:border-ink-600 dark:bg-ink-900 dark:text-ink-100 rounded-md p-2 text-sm min-h-[200px] font-mono" />
              </div>
            </div>
