...
type Post = { id: string; title: string; body: string; created_at: number; author_id: string; author_name: string; author_image: string | null; author_avatar_frame: string | null; score: number; my_vote: number; comment_count: number };
type Announcement = { id: string; author_id: string; author_name: string; author_image: string | null; author_avatar_frame: string | null; room_id: string | null; title: string; body: string; is_pinned: number; created_at: number };
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-ink-400 dark:text-ink-500 mt-1 flex items-center gap-2">
                      <Link href={`/u/${a.author_id}`}>
                        <LaurelFrame frameId={a.author_avatar_frame || 'none'} size={24}>
                          {a.author_image ? <img src={a.author_image} className="w-4 h-4 rounded-full" /> : <span className="w-4 h-4 rounded-full bg-accent-500 text-white text-[8px] font-medium flex items-center justify-center">{(a.author_name || '?').slice(0,1).toUpperCase()}</span>}
                        </LaurelFrame>
                      </Link>
                      <span>by <Link href={`/u/${a.author_id}`} className="hover:underline">{a.author_name}</Link></span>
                      <span>·</span>
        ...
        <ul className="grid sm:grid-cols-3 gap-2">
          {members.map((m) => (
            <li key={m.id} className="rounded-lg border border-ink-100 dark:border-ink-700 bg-white dark:bg-ink-800 p-2 flex items-center gap-2 text-sm">
              <LaurelFrame frameId={m.avatar_frame || 'none'} size={28}>
                {m.image ? <img src={m.image} className="w-5 h-5 rounded-full" /> : <span className="w-5 h-5 rounded-full bg-accent-500 text-white text-[9px] font-medium flex items-center justify-center">{(m.name || '?').slice(0, 1).toUpperCase()}</span>}
              </LaurelFrame>
            ...
            {posts.map((p) => (
              <li key={p.id} className="rounded-xl border border-ink-100 dark:border-ink-700 bg-white dark:bg-ink-800 p-3 flex gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-ink-400 dark:text-ink-500 mt-0.5 flex items-center gap-2">
        ...
        <div className="fixed inset-0 z-50 bg-ink-900/40 flex items-center justify-center p-4" onClick={() => setShowNew(false)}>
          <div className="bg-white dark:bg-ink-800 rounded-2xl shadow-2xl max-w-xl w-full p-5" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Title" className="w-full border border-ink-200 dark:border-ink-600 bg-white bg-white bg-white dark:bg-ink-900 dark:text-ink-100 rounded-md p-2 text-sm" />
              <textarea value={body} onChange={(e) => setBody(e.target.value)} placeholder="Body (Markdown OK)" className="w-full border border-ink-200 dark:border-ink-600 bg-white bg-white dark:bg-ink-900 dark:text-ink-100 rounded-md p-2 text-sm min-h-[120px] font-mono" />
              <input value={tags} onChange={(e) => setTags(e.target.value)} placeholder="tags (comma separated)" className="w-full border border-ink-200 dark:border-ink-600 bg-white bg-white bg-white dark:bg-ink-900 dark:text-ink-100 rounded-md p-2 text-sm" />
            </div>
          ...
          <div className="bg-white dark:bg-ink-800 rounded-2xl shadow-2xl max-w-xl w-full p-5" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input value={annTitle} onChange={(e) => setAnnTitle(e.target.value)} className="w-full border border-ink-200 dark:border-ink-600 bg-white bg-white bg-white dark:bg-ink-900 dark:text-ink-100 rounded-md p-2 text-sm" />
              </div>
              <div>
