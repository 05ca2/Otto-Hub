...
import { ArrowUp, ArrowDown, ArrowLeft, Loader2, Send, Sparkles, MessageSquare, Reply, Pencil, Trash2, X } from 'lucide-react';
...
export default function ExploreItemDetailPage() {
  ...
  function startEdit() {
    ...
    try { setEditTags(item.tags?.join(', ') || ''); } catch { setEditTags(''); }
  }
  ...
  async function saveEdit() {
    ...
    setEditSaving(true);
    try {
      const r = await fetch(`/api/posts/${item.id}`, {
        method: 'PATCH',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ title: editTitle, body: editBody, tags: editTags.split(',').map((s) => s.trim()).filter(Boolean) }),
      });
  ...
  async function deletePost() {
    if (!item) return;
    const r = await fetch(`/api/posts/${item.id}`, { method: 'DELETE' });
    if (r.ok) router.push('/explore');
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <article className="rounded-2xl border border-ink-100 dark:border-ink-600 bg-white dark:bg-ink-800 p-6">
        <div className="flex gap-4">
          <div className="flex flex-col items-center text-sm text-ink-600 dark:text-ink-400 w-10 shrink-0">
            <button onClick={() => vote('item', item.id, item.my_vote === 1 ? 0 : 1)} className={`p-1 rounded hover:bg-ink-100 dark:hover:bg-ink-700 ${item.my_vote === 1 ? 'text-accent-600' : ''}`} aria-label="Upvote"><ArrowUp className="w-5 h-5" /></button>
            <span className="tabular-nums">{item.score}</span>
            <button onClick={() => vote('item', item.id, item.my_vote === -1 ? 0 : -1)} className={`p-1 rounded hover:bg-ink-100 dark:hover:bg-ink-700 ${item.my_vote === -1 ? 'text-red-500' : ''}`} aria-label="Downvote"><ArrowDown className="w-5 h-5" /></button>
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-1">
              <h1 className="text-2xl font-semibold dark:text-ink-100 flex-1">{item.title}</h1>
              {type === 'post' && me && item.author_id === me.id && (
                <div className="flex items-center gap-1 shrink-0">
                  <button onClick={startEdit} className="p-1.5 rounded hover:bg-ink-100 dark:hover:bg-ink-700 text-ink-400 dark:text-ink-500" aria-label="Edit post"><Pencil className="w-4 h-4" /></button>
                  <button onClick={() => setDeleting(true)} className="p-1.5 rounded hover:bg-red-50 dark:hover:bg-red-900/30 text-ink-400 dark:text-ink-500 hover:text-red-500" aria-label="Delete post"><Trash2 className="w-4 h-4" /></button>
                </div>
              )}
            </div>
            <div className="text-xs text-ink-400 mt-1.5 flex flex-wrap items-center gap-2">
              ...
              {item.tags && item.tags.length > 0 && item.tags.map((t) => (
                <span key={t} className="px-1.5 py-0.5 rounded bg-ink-100 dark:bg-ink-700 text-ink-600 dark:text-ink-300">#{t}</span>
              ))}
            </div>
      ...
      <section>
        <h2 className="text-lg font-semibold mb-3 dark:text-ink-100">
        ...
        {type === 'post' && (
          <div className="mt-4 rounded-xl border border-ink-100 dark:border-ink-600 bg-white dark:bg-ink-800 p-4">
            <textarea
              value={body}
              onChange={(e) => setBody(e.target.value)}
              placeholder="Write a comment (Markdown OK)…"
              className="w-full border border-ink-200 dark:border-ink-600 rounded-md p-2 text-sm min-h-[80px] font-mono bg-white dark:bg-ink-900 dark:text-ink-100"
            />
      ...
      {editing && (
        <div className="fixed inset-0 z-50 bg-white dark:bg-ink-900 flex flex-col">
          <div className="flex items-center justify-between px-4 py-3 border-b border-ink-200 dark:border-ink-700">
            <h3 className="font-semibold dark:text-ink-100">Edit post</h3>
            <button onClick={() => setEditing(false)} className="p-1.5 rounded hover:bg-ink-100 dark:hover:bg-ink-700"><X className="w-5 h-5 dark:text-ink-300" /></button>
          </div>
          <div className="flex-1 overflow-auto p-6">
            <div className="max-w-3xl mx-auto space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-1 dark:text-ink-200">Tags (comma separated)</label>
                <input value={editTags} onChange={(e) => setEditTags(e.target.value)} placeholder="biology, photosynthesis" className="w-full border border-ink-200 dark:border-ink-600 dark:bg-ink-800 dark:text-ink-100 rounded-md p-2.5 text-sm" />
              </div>
            </div>
          </div>
      ...
      {/* Delete confirmation */}
      {deleting && (
        <div className="fixed inset-0 z-50 bg-ink-900/40 flex items-center justify-center p-4" onClick={() => setDeleting(false)}>
          <div className="bg-white dark:bg-ink-800 rounded-2xl shadow-2xl max-w-sm w-full p-5" onClick={(e) => e.stopPropagation()}>
            <h3 className="font-semibold dark:text-ink-100">Delete post?</h3>
            <p className="text-sm text-ink-600 dark:text-ink-400 mt-2">This will permanently delete &quot;{item.title}&quot; and all its comments.</p>
            <div className="flex justify-end gap-2 mt-4">
              <button onClick={() => setDeleting(false)} className="px-3 py-1.5 rounded text-sm hover:bg-ink-100 dark:hover:bg-ink-700">Cancel</button>
              <button onClick={deletePost} className="px-3 py-1.5 rounded bg-red-500 text-white text-sm font-medium hover:bg-red-600">Delete</button>
            </div>
          </div>
        </div>
      )}
...
function CommentItem({ 
  ...
  return (
    <div className={depth > 0 ? 'ml-6 mt-2' : ''}>
      <div className="rounded-xl border border-ink-100 dark:border-ink-600 bg-white dark:bg-ink-800 p-4 flex gap-3">
        <div className="flex flex-col items-center text-sm text-ink-600 dark:text-ink-400 w-10 shrink-0">
          <button onClick={() => vote('comment', comment.id, comment.my_vote === 1 ? 0 : 1)} className={`p-1 rounded hover:bg-ink-100 dark:hover:bg-ink-700 ${comment.my_vote === 1 ? 'text-accent-600' : ''}`} aria-label="Upvote comment"><ArrowUp className="w-4 h-4" /></button>
          <span className="tabular-nums">{comment.score}</span>
          <button onClick={() => vote('comment', comment.id, comment.my_vote === -1 ? 0 : -1)} className={`p-1 rounded hover:bg-ink-100 dark:hover:bg-ink-700 ${comment.my_vote === -1 ? 'text-red-500' : ''}`} aria-label="Downvote comment"><ArrowDown className="w-4 h-4" /></button>
        </div>
        <div className="flex-1 min-w-0">
          ...
          {/* Reply input */}
          {replyTo === comment.id && (
            <div className="mt-3 rounded-lg border border-ink-200 dark:border-ink-600 bg-ink-50 dark:bg-ink-700 p-3">
              <textarea
                value={replyBody}
                onChange={(e) => setReplyBody(e.target.value)}
                placeholder="Write a reply (Markdown OK)…"
                className="w-full border border-ink-200 dark:border-ink-600 rounded-md p-2 text-sm min-h-[60px] font-mono bg-white dark:bg-ink-900 dark:text-ink-100"
                autoFocus
              />
              <div className="flex justify-end gap-2 mt-2">
                <button onClick={() => { setReplyTo(null); setReplyBody(''); }} className="px-2 py-1 text-xs rounded hover:bg-ink-100 dark:hover:bg-ink-600 dark:text-ink-300">Cancel</button>
                <button 
