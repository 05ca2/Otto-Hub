...
export async function GET(_req: NextRequest, { params }: Params) {
  ...
  const post = db
    ...
    .get(params.id) as (PostRow & { author_name: string; author_image: string | null; author_avatar_frame: string | null }) | undefined;
  ...
  const comments = db
    .prepare(
      `SELECT c.*, u.name AS author_name, u.image AS author_image, u.avatar_frame AS author_avatar_frame
...
// Content moderation check if editing to public visibility
   if (post.visibility === 'public') {
     const newTitle = parsed.data.title ?? post.title;
     const newBody = parsed.data.body ?? post.body;
     const modResult = await moderateContent(newTitle, newBody);
     if (!modResult.ok) {
       try {
         const vId = (await import('nanoid')).nanoid();
         const preview = ((newBody || '').slice(0, 200) || '').replace(/'/g, "''");
         const reason = (modResult.reason || 'Content violates guidelines').replace(/'/g, "''");
         const db = getDb();
         db.prepare(`INSERT INTO violations (id, post_id, author_id, reason, content_preview, source, created_at) VALUES (?, ?, ?, ?, ?, 'moderation', ?)`).run(vId, params.id, user.id, reason, preview.slice(0, 200), Date.now());
       } catch {}
