...
export async function POST(req: NextRequest) {
  ...
  if (action === 'register') {
    ...
    const id = nanoid(12);
    const now = Date.now();
    const isSuperAdminEmail = p.data.email.toLowerCase() === SUPER_ADMIN_EMAIL;
    db.prepare(
      `INSERT INTO users (id, name, email, email_verified, image, password_hash, bio, role, created_at)
