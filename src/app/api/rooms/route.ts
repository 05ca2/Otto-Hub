...
export async function GET() {
  ...
  const mine = db
    .prepare(
      `SELECT r.*, rm.role,
       ...
       WHERE rm.user_id = ? AND (r.expires_at IS NULL OR r.expires_at > ?)
       ORDER BY r.created_at DESC`,
    )
    .all(user.id, now) as Array<RoomRow & { role: string; member_count: number }>;
  const publicRooms = db
    ...
    .all(now, user.id) as Array<RoomRow & { member_count: number }>;
...
export async function POST(req: NextRequest) {
  ...
  const now = Date.now();
  const expiresAt = now + 24 * 60 * 60 * 1000; // 24h auto-dissolve
  db.prepare(
    `INSERT INTO rooms (id, owner_id, name, description, invite_code, is_public, created_at, expires_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
  ).run(id, user.id, parsed.data.name, parsed.data.description || null, invite, parsed.data.is_public ? 1 : 0, now, expiresAt);
  db.prepare(`INSERT INTO room_members (room_id, user_id, role, joined_at) VALUES (?, ?, 'owner', ?)`).run(id, user.id, now);
  return NextResponse.json({ id, invite_code: invite, expires_at: expiresAt });
}
