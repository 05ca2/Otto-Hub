...
const COLOR_OPTIONS: Array<{ value: string; labelKey: string; swatch: string }> = [
  { value: 'accent', labelKey: 'countdown.color', swatch: 'bg-accent-500' },
  { value: 'green', labelKey: 'countdown.color', swatch: 'bg-emerald-500' },
  { value: 'red', labelKey: 'countdown.color', swatch: 'bg-rose-500' },
  { value: 'yellow', labelKey: 'countdown.color', swatch: 'bg-amber-400' },
  { value: 'blue', labelKey: 'countdown.color', swatch: 'bg-sky-500' },
];
...
export function CountdownWidget() {
  ...
  async function onDelete(id: string) {
    if (!confirm(locale === 'zh' ? '删除此倒计时事件？' : 'Delete this countdown event?')) return;
    setError(null);
    try {
    ...
    return (
      <section className="rounded-xl border border-ink-100 dark:border-ink-700 bg-white dark:bg-ink-800 p-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h3 className="font-semibold text-sm">{t(locale, 'countdown.title')}</h3>
        </div>
        <p className="text-xs text-ink-500 dark:text-ink-400">
          {locale === 'zh' ? '登录以追踪即将到来的考试和活动。' : 'Sign in to track upcoming exams and events.'}
        </p>
      </section>
    );
  }
    ...
    <section className="rounded-xl border border-ink-100 dark:border-ink-700 bg-white dark:bg-ink-800 p-4">
      <header className="flex items-center justify-between mb-3">
        ...
        >
          ...
          {showForm ? t(locale, 'common.cancel') : t(locale, 'countdown.add')}
        </button>
      </header>
        ...
        <form onSubmit={onAdd} className="space-y-2 mb-3 p-3 rounded-lg bg-ink-50 dark:bg-ink-700/50">
          <div>
            <label className="block text-[11px] uppercase tracking-wide text-ink-500 dark:text-ink-400 mb-1">
              {t(locale, 'countdown.eventTitle')}
            </label>
            <input
              type="text"
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              placeholder={locale === 'zh' ? '期末考试、项目截止日期…' : 'Final exam, project deadline…'}
              maxLength={120}
              required
            ...
            <div className="flex flex-wrap gap-1.5">
              {COLOR_OPTIONS.map((opt) => (
                <button
                  ...
                  title={t(locale, 'countdown.color')}
                  aria-label={t(locale, 'countdown.color')}
                />
              ))}
            </div>
          </div>
            ...
            return (
              ...
              >
                ...
                <div className={`text-right shrink-0 ${urgencyClass(days)}`}>
                  ...
                  <div className="text-[10px] uppercase tracking-wide">{formatDays(days, locale)}</div>
                </div>
                <button
