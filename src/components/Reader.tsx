...
import { MessageCircle, Loader2, Send, X, Bot, User, Maximize2, Minimize2 } from 'lucide-react';
...
export function Reader({
  ...
  const [genFullscreen, setGenFullscreen] = useState(false);
  ...
  return (
    <div className="grid lg:grid-cols-[1fr_420px] gap-6">
      ...
      <aside className="space-y-4">
        {/* Chat Window */}
        {chatFullscreen && <div className="fixed inset-0 z-40 bg-ink-900/30 backdrop-blur-sm" onClick={() => setChatFullscreen(false)} />}
        <div className={`rounded-2xl border border-ink-200 dark:border-ink-600 bg-white dark:bg-ink-800 p-4 flex flex-col ${chatFullscreen ? 'fixed inset-4 z-50 shadow-2xl' : 'h-[500px]'}`}>
          <div className="flex items-center gap-2 mb-3 pb-3 border-b border-ink-200 dark:border-ink-600">
            ...
            <button
              ...
              className="p-1 rounded hover:bg-ink-100 dark:hover:bg-ink-700"
        ...
        {/* Generation Section */}
        {genFullscreen && <div className="fixed inset-0 z-40 bg-ink-900/30 backdrop-blur-sm" onClick={() => setGenFullscreen(false)} />}
        <div className={`rounded-2xl border border-ink-200 dark:border-ink-600 bg-white dark:bg-ink-800 p-4 ${genFullscreen ? 'fixed inset-4 z-50 overflow-auto shadow-2xl' : ''}`}>
          <div className="flex items-center gap-2 mb-3">
            <h3 className="text-sm font-semibold dark:text-ink-100">Generate</h3>
            <button
              onClick={() => setGenFullscreen(!genFullscreen)}
              className="p-1 rounded hover:bg-ink-100 dark:hover:bg-ink-700 ml-auto"
              title={genFullscreen ? 'Exit fullscreen' : 'Fullscreen'}
            >
              {genFullscreen ? <Minimize2 className="w-4 h-4" /> : <Maximize2 className="w-4 h-4" />}
            </button>
          </div>
          ...
          {activeGen ? (
            activeGen.kind === 'questions' ? (
              ...
              <div className="markdown-body text-sm max-h-[60vh] overflow-y-auto pr-1 dark:text-ink-200">
                <MarkdownRenderer>{activeGen.content}</MarkdownRenderer>
              </div>
            )
          ) : (
